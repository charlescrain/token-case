...
taskList :: forall props eff. T.Spec (now :: NOW | eff) TaskListState props TaskListAction
taskList = container $ fold
  ...
  where
    ...
    container :: forall state action. T.Spec (now :: NOW | eff) state props action -> T.Spec (now :: NOW | eff) state props action
    ...
    header :: T.Spec (now :: NOW | eff) TaskListState props TaskListAction
    header = T.simpleSpec performAction render
      where
        ...
        performAction :: T.PerformAction (now :: NOW | eff) TaskListState props TaskListAction
        performAction (NewTask s) _ _ = void $ do
          (LocalValue _ dt) <- liftEff $ nowDateTime 
          T.modifyState $ \state ->
            state { tasks = Cons (initialTask s dt) state.tasks
                  , editText = ""
                  }
    ...
    applyFilter :: forall action. Filter -> T.Spec (now :: NOW | eff) Task props action -> T.Spec (now :: NOW | eff) Task props action
    ...
    table :: T.Spec (now :: NOW | eff) TaskListState props TaskListAction -> T.Spec (now :: NOW | eff) TaskListState props TaskListAction
    ...
    footer :: forall action. T.Spec (now :: NOW | eff) TaskListState props action
    ...
    listActions :: T.Spec (now :: NOW | eff) TaskListState props TaskListAction
